...
def confusion_matrix_func(preds, target):
    ...
    df_cm = pd.DataFrame(data, columns=np.unique(target_res), index=np.unique(target_res))
...
def main():   
    with wandb.init(project = 'cola_model', config = new_config):
        ...
        for epoch in range(wandb.config["epochs"]):
            cola_model.train()
            total_acc_train, total_loss_train, total_train_pre_score, total_train_rec_score, total_train_f1 = 0, 0, 0, 0, 0
            for train_input in tqdm(train_dataloader):
                cola_model.zero_grad() 
                loss, acc, train_pre_score, train_rec_score, train_f1 = cola_model.training_step(train_input)
            ...
            total_val_acc, total_val_loss, total_val_pre_score, total_val_rec_score, total_val_f1 = 0, 0, 0, 0, 0
            preds_res, target_res = [], []
            for val_input in tqdm(val_dataloader):
                with torch.no_grad():   
                    val_loss, val_acc, val_pre_score, val_rec_score, val_f1, preds, target = cola_model.validation_step(val_input)
            ...
            logger.info(f"train_loss: {total_loss_train / len(train_dataloader)} || train_acc: {total_acc_train / len(train_dataloader)} || val_loss: {total_val_loss / len(val_dataloader)} || val_acc: {total_val_acc / len(val_dataloader)}")
            if epoch % wandb.config["save_frequency"] == 0 and wandb.config["save_model"] is True:
