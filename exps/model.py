...
class ColaModel(nn.Module):
    def __init__(
        ...
        self.W = nn.Linear(
            ...
            int(config["model_layer"]["linear_output_dim"]),
        )
    ...
    def training_step(self, batch):
        ...
        train_acc, train_pre_score, train_rec_score, train_f1 = self.calculate_metric(
            preds, batch["label"].cpu()
        )
    ...
    def validation_step(self, batch):
        ...
        for ele in logits:
            if ele > config["threshold"]:
                ...
                preds.append(0)
        preds = np.array(preds)
        val_acc, val_pre_score, val_rec_score, val_f1 = self.calculate_metric(
            preds, batch["label"].cpu()
        )
        return (
            val_loss,
